...
//    isFlipping and flipStartTime variables to track the flipping state and the time when flipping starts. The update method checks for user input to initiate flipping and calculates the flip time when flipping is in progress. If the flip time falls within a certain range (e.g., between 0.2 and 1.0 seconds), it considers it a successful flip and calls the collectCherry method
